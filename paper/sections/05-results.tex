\section{Resultados}
\label{sec:results}

\subsection{Comparação das alocações}

A Tabela~\ref{tab:allocations} contrasta as três alocações para $n=378$. Note
que a alocação proporcional concede apenas $1$ exemplo aos estratos mais
raros (\textit{racism}, \textit{xenophobia}); a alocação de Neyman, ponderada
pelo desvio-padrão de $y$ no estrato, redistribui o esforço amostral em
direção aos estratos com maior variabilidade interna, em particular
\textit{obscene} e \textit{insult}.

\begin{table}[!t]
\centering
\small
\caption{Alocação de $n=378$ entre estratos sob três regimes (ToLD-Br).
Os desvios-padrão $\sigma_h$ referem-se à variável-alvo
$y=\mathtt{total\_severity}$.}
\label{tab:allocations}
\begin{tabular}{lrrrrr}
\toprule
Estrato       & $N_h$ & $\sigma_h$ & $n_h$ Prop. & $n_h$ Neyman & $n_h$ Fix~B \\
\midrule
\textit{racism}     &     33 & $1{,}96$ &   1 &   2 &   31 \\
\textit{xenophobia} &     42 & $2{,}43$ &   1 &   3 &   38 \\
\textit{misogyny}   &    129 & $1{,}53$ &   2 &   5 &   97 \\
\textit{homophobia} &    172 & $1{,}57$ &   3 &   7 &  120 \\
\textit{insult}     & 1{,}755 & $1{,}13$ &  31 &  55 &  316 \\
\textit{obscene}    & 1{,}932 & $0{,}69$ &  35 &  37 &  321 \\
clean              & 16{,}937 & $0{,}57$ & 305 & 269 &  376 \\
\midrule
\textbf{Total}     &       &        & \textbf{378} & \textbf{378} & \textbf{1{,}299} \\
\bottomrule
\end{tabular}
\end{table}

A heterogeneidade observada de $\sigma_h$ --- de $0{,}57$ no estrato
\textit{clean} a $2{,}43$ em \textit{xenophobia} --- justifica empiricamente
o uso da alocação ótima de Neyman: pela equação~(\ref{eq:gain-neyman}), o
ganho de variância sobre a proporcional é estritamente positivo sempre que
os $\sigma_h$ diferem materialmente.

A coluna \textbf{Fix~B} reporta a alocação Cochran-por-estrato com censo dos
estratos pequenos (Cochran~\cite{cochran1977}~§5.5), apresentada em
\S\ref{sec:method:allocation} como referência alternativa: ela atinge precisão
de $5\%$ \textit{em cada estrato} ao custo de $1{,}299$ exemplos
($3{,}4\times$ o orçamento de Neyman). Por destinar-se a um regime de
auditoria diferente (precisão por estrato em vez de mínima variância da média
populacional), Fix~B não é diretamente comparável aos estimadores de
$\bar{Y}$ avaliados nas Seções~\ref{sec:results:single}--\ref{sec:results:mc}
e é, portanto, mantida apenas como referência conceitual.

\subsection{Estimativas de $\bar{Y}$ sob alocação de Neyman --- realização única}
\label{sec:results:single}

A Tabela~\ref{tab:estimators} apresenta os cinco estimadores aplicados a
\emph{uma} amostra Neyman (semente \texttt{random\_state=42}), com intervalos
de confiança de $95\%$ e erro absoluto contra a verdade populacional
$\bar{Y}=0{,}8591$. Os números desta tabela são ilustrativos --- a
realização-única exibe variabilidade amostral; a evidência consolidada vem da
replicação Monte Carlo da \S\ref{sec:results:mc}.

\begin{table}[!t]
\centering
\small
\setlength{\tabcolsep}{3pt}
\caption{Estimadores estratificados de $\bar{Y}$ sob Neyman ($n=378$).}
\label{tab:estimators}
\begin{tabular}{lcccc}
\toprule
Estimador                    & Estimativa & EP    & IC 95\%             & $|\hat\theta - \bar{Y}|$ \\
\midrule
Média estratificada           & $0{,}8676$ & $0{,}0345$ & $[0{,}80,\,0{,}94]$ & $0{,}0085$ \\
Razão separada                & $0{,}9203$ & $0{,}0563$ & $[0{,}81,\,1{,}03]$ & $0{,}0611$ \\
Razão combinada               & $0{,}8532$ & $0{,}0489$ & $[0{,}76,\,0{,}95]$ & $\mathbf{0{,}0059}$ \\
Regressão separada            & $0{,}8787$ & $\mathbf{0{,}0342}$ & $[0{,}81,\,0{,}95]$ & $0{,}0196$ \\
Regressão combinada           & $0{,}8672$ & $0{,}0345$ & $[0{,}80,\,0{,}93]$ & $0{,}0080$ \\
\midrule
\textbf{Verdade populacional} & $\mathbf{0{,}8591}$ & --      & --                  & $0{,}0000$ \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Observações da realização única.} Todos os cinco intervalos contêm a
verdade populacional $\bar{Y}=0{,}8591$, atestando a calibração dos
estimadores. Nesta realização específica, o \textit{razão combinado} apresenta
o menor erro absoluto ($0{,}0059$); este resultado, contudo, deve ser
interpretado com cautela --- a \S\ref{sec:results:mc} demonstra que sob
$B=500$ replicações o estimador de menor EQM esperado é a média
estratificada (empatada com a regressão combinada), e não o razão combinado
isoladamente.

\subsection{Comparação Neyman vs proporcional}

A Tabela~\ref{tab:se-reduction} quantifica a redução de erro-padrão obtida
pela alocação ótima de Neyman frente à alocação proporcional, mantendo
$n=378$.

\begin{table}[!t]
\centering
\caption{Redução percentual no erro-padrão: Neyman vs proporcional.}
\label{tab:se-reduction}
\begin{tabular}{lrrr}
\toprule
Estimador          & EP Proporc. & EP Neyman & Redução \\
\midrule
Média estratificada & $0{,}0377$ & $0{,}0345$ & $\mathbf{8{,}3\%}$  \\
Razão separada      & $0{,}0704$ & $0{,}0563$ & $\mathbf{20{,}0\%}$ \\
Razão combinada     & $0{,}0500$ & $0{,}0489$ & $2{,}3\%$  \\
Regressão separada  & $0{,}0369$ & $0{,}0342$ & $7{,}2\%$  \\
Regressão combinada & $0{,}0376$ & $0{,}0345$ & $8{,}2\%$  \\
\bottomrule
\end{tabular}
\end{table}

A redução é substancial sobretudo para a \textit{razão separada} ($20\%$),
estimador particularmente sensível ao número de exemplos por estrato. Para a
\textit{razão combinada}, a redução é apenas marginal --- o esquema combinado
amortece a sensibilidade à alocação.

\subsection{Visualização}

A Figura~\ref{fig:forest} apresenta o \textit{forest plot} dos cinco
estimadores com seus IC de $95\%$, sobrepostos à verdade populacional
(linha vermelha tracejada).

\begin{figure}[!t]
\centering
\includegraphics[width=.85\linewidth]{estimators_comparison_neyman.png}
\caption{Estimativas de $\bar{Y}$ com IC 95\% sob alocação de Neyman.}
\label{fig:forest}
\end{figure}

\subsection{Replicação Monte Carlo ($B=500$)}
\label{sec:results:mc}

Para distinguir variabilidade de uma única realização aleatória do
comportamento esperado dos estimadores, replicamos o desenho amostral
sob cada alocação por $B=500$ sementes independentes. A
Tabela~\ref{tab:montecarlo} reporta o viés empírico, o desvio-padrão
empírico das estimativas pontuais, a média do erro-padrão analítico, a
cobertura empírica do IC $95\%$ e o erro quadrático médio (EQM).

\begin{table*}[!t]
\centering
\small
\setlength{\tabcolsep}{6pt}
\caption{Replicação Monte Carlo ($B=500$): EQM e cobertura.}
\label{tab:montecarlo}
\begin{tabular}{llrrrrr}
\toprule
Alocação & Estimador  & Viés & SD emp. & EP an. & Cob. 95\% & EQM \\
\midrule
Neyman   & média estr.       & $-0{,}0005$ & $0{,}0339$ & $0{,}0331$ & $0{,}934$ & $\mathbf{0{,}00115}$ \\
Neyman   & razão sep.        &  $0{,}0131$ & $0{,}0549$ & $0{,}0524$ & $0{,}934$ & $0{,}00319$ \\
Neyman   & razão comb.       &  $0{,}0003$ & $0{,}0471$ & $0{,}0468$ & $\mathbf{0{,}956}$ & $0{,}00222$ \\
Neyman   & regressão sep.    & $-0{,}0021$ & $0{,}0388$ & $0{,}0327$ & $0{,}896$ & $0{,}00151$ \\
Neyman   & regressão comb.   & $-0{,}0004$ & $0{,}0339$ & $0{,}0331$ & $0{,}938$ & $\mathbf{0{,}00115}$ \\
\midrule
Proporc. & média estr.       &  $0{,}0001$ & $0{,}0365$ & $0{,}0343$ & $0{,}934$ & $0{,}00133$ \\
Proporc. & razão sep.        &  $0{,}0348$ & $0{,}0663$ & $0{,}0589$ & $0{,}876$ & $0{,}00561$ \\
Proporc. & razão comb.       &  $0{,}0017$ & $0{,}0470$ & $0{,}0469$ & $0{,}950$ & $0{,}00221$ \\
Proporc. & regressão sep.    & $-0{,}0024$ & $0{,}0517$ & $0{,}0330$ & $0{,}836$ & $0{,}00268$ \\
Proporc. & regressão comb.   &  $0{,}0001$ & $0{,}0367$ & $0{,}0343$ & $0{,}932$ & $0{,}00135$ \\
\bottomrule
\end{tabular}
\end{table*}

\textbf{Conclusões da Monte Carlo.} (i) Todos os estimadores, exceto razão
separada, exibem viés empírico desprezível (\(<10^{-3}\)). A razão separada
herda o viés clássico de razão de Cochran (\S6.10). (ii) O EP analítico de
grande amostra é bem calibrado para a média estratificada e estimadores
combinados (razão SD empírico/EP analítico próxima de $1$); subestima a
variância das versões \textit{separadas} sob proporcional (cobertura
$83{,}6\%$ contra $95\%$ nominal). (iii) Neyman reduz o EQM frente à
proporcional em \emph{quatro dos cinco} estimadores: $13{,}5\%$ na média
estratificada, $43{,}1\%$ na razão separada, $43{,}7\%$ na regressão
separada, $14{,}8\%$ na regressão combinada. A única exceção é a razão
combinada, em que a diferença ($-0{,}5\%$) não é estatisticamente significativa
(Wilcoxon $p=0{,}18$, Tabela~\ref{tab:wilcoxon}). (iv) Sob Neyman, a média
estratificada e a regressão combinada empatam pelo menor EQM ($0{,}00115$); a
razão combinada apresenta a melhor cobertura ($95{,}6\%$).

\subsubsection*{Significância estatística da redução de erro-padrão}

Para testar formalmente $H_0: \mathrm{EP}_{\mathrm{prop}} = \mathrm{EP}_{\mathrm{Ney}}$
contra $H_1: \mathrm{EP}_{\mathrm{prop}} > \mathrm{EP}_{\mathrm{Ney}}$,
aplicamos o teste de Wilcoxon de postos com sinal pareado sobre os $B=500$
pares de erros-padrão observados em cada estimador
(Tabela~\ref{tab:wilcoxon}). A escolha do teste não-paramétrico evita supor
normalidade da distribuição amostral dos erros-padrão.

\begin{table}[!t]
\centering
\small
\setlength{\tabcolsep}{3pt}
\caption{Teste de Wilcoxon pareado ($H_1$: EP proporcional $>$ EP Neyman),
$B=500$ replicações.}
\label{tab:wilcoxon}
\begin{tabular}{lrrr}
\toprule
Estimador & EP prop. (med.) & EP Ney. (med.) & valor-$p$ \\
\midrule
média estr.       & $0{,}0342$ & $0{,}0331$ & $\mathbf{1{,}2 \times 10^{-51}}$ \\
razão sep.        & $0{,}0589$ & $0{,}0524$ & $\mathbf{2{,}4 \times 10^{-83}}$ \\
razão comb.       & $0{,}0469$ & $0{,}0468$ & $0{,}176$ (n.s.) \\
regressão sep.    & $0{,}0330$ & $0{,}0327$ & $\mathbf{7{,}5 \times 10^{-12}}$ \\
regressão comb.   & $0{,}0342$ & $0{,}0330$ & $\mathbf{3{,}4 \times 10^{-51}}$ \\
\bottomrule
\end{tabular}
\end{table}

Os resultados confirmam que a vantagem de Neyman é estatisticamente
significativa para quatro dos cinco estimadores ($p < 10^{-11}$). A única
exceção é o estimador razão combinado ($p=0{,}18$), em que o esquema de
agregação pré-razão amortece a sensibilidade ao desenho amostral, conforme
discutido em \S\ref{sec:theory:estimators}. Esta heterogeneidade tem
implicação prática: a alocação ótima é especialmente recomendada quando se
adota razão separada ou regressão combinada.

\subsection{Diagnóstico de ruído de rotulagem}

A Tabela~\ref{tab:noise} apresenta a contagem de \textit{tweets} em cada nível
de votos por categoria, em particular a fração com exatamente um voto
($\textit{disputed share}$) --- a zona descartada pela regra de maioria mas
não classificada como negativa pelos anotadores. Para
categorias como \textit{obscene}, $20{,}2\%$ da população está nesta zona
ambígua, sugerindo que a binarização por maioria pode subestimar
sistematicamente a prevalência efetiva de toxicidade.

\begin{table*}[!t]
\centering
\small
\setlength{\tabcolsep}{8pt}
\caption{Diagnóstico de votos disputados (Camada 3.1).}
\label{tab:noise}
\begin{tabular}{lrrrrrr}
\toprule
Categoria & Votos 0 & Votos 1 & Votos 2 & Votos 3 & Posit. ($\geq 2$) & \% Disputado \\
\midrule
\textit{homophobia} & 20{,}656 & 168 & 102 & 74 & 176 & 0{,}80\% \\
\textit{obscene}    & 14{,}348 & 4{,}249 & 1{,}791 & 612 & 2{,}403 & \textbf{20{,}23\%} \\
\textit{insult}     & 16{,}615 & 2{,}516 & 1{,}352 & 517 & 1{,}869 & \textbf{11{,}98\%} \\
\textit{racism}     & 20{,}862 & 105 & 27 & 6 & 33 & 0{,}50\% \\
\textit{misogyny}   & 20{,}537 & 330 & 104 & 29 & 133 & 1{,}57\% \\
\textit{xenophobia} & 20{,}849 & 109 & 27 & 15 & 42 & 0{,}52\% \\
\bottomrule
\end{tabular}
\end{table*}

\subsection{Discussão}

\paragraph{Q1 — tamanho amostral.} A fórmula de Cochran com FPC dá $n=378$
sobre $N=21{,}000$, suficiente para erro de amostragem de $5\%$ a $95\%$ de
confiança no caso conservador $p=0{,}5$. Este número, frequentemente omitido
em trabalhos correlatos, é diretamente derivável e justificável.

\paragraph{Q2 — ganho de Neyman.} A redução de erro-padrão obtida por Neyman
é mais expressiva quando o estimador é sensível à variabilidade entre
estratos: $20\%$ na razão separada, $8\%$ na média estratificada e na
regressão. Esta heterogeneidade reforça a recomendação prática de privilegiar
estimadores combinados quando a alocação não puder ser otimamente derivada.

\paragraph{Q3 — razão vs regressão.} A análise Monte Carlo
(Tabela~\ref{tab:montecarlo}) mostra que, sob Neyman, a média estratificada
simples e a regressão combinada empatam pelo menor EQM ($0{,}00115$), seguidas
de perto pela regressão separada ($0{,}00151$); o razão combinado vem em
quarto ($0{,}00222$) e o razão separado em último ($0{,}00319$). A
regressão e a média estratificada são, portanto, equivalentes sob este
desenho. Esse resultado --- aparentemente decepcionante para a regressão ---
é coerente com a baixíssima correlação populacional
$\rho_{xy} \approx 0{,}011$ entre \texttt{word\_count} e
\texttt{total\_severity}, que explica apenas $\rho_{xy}^{2} \approx 0{,}01\%$
da variância de $y$. Vale notar que as correlações \emph{intra-estrato} são
maiores --- até $0{,}27$ em \textit{racism} ---, mas o estrato dominante
\textit{clean} ($N_h \approx 17$ mil) puxa a correlação populacional para
próximo de zero. Em outras palavras: o auxiliar adotado é demasiado fraco
para que a regressão extraia ganho material sobre a média estratificada, e o
ganho prometido pela teoria (Cochran~\cite{cochran1977}~§7) só se manifesta
com $\rho_{xy}$ substancialmente maior. Esta é uma limitação da escolha
experimental deste artigo, não da teoria.

\paragraph{Limitações.} (i) Apenas \texttt{word\_count} é avaliada como
auxiliar; \textit{features} mais ricas (\texttt{caps\_ratio},
\texttt{punc\_density}, embeddings) podem produzir maior $\rho_{xy}$ e maior
ganho da regressão. (ii) A binarização por maioria descarta a granularidade
ordinal das anotações; o diagnóstico de ruído (Tabela~\ref{tab:noise}) sugere
que esta perda é não-trivial para categorias como \textit{obscene}. (iii) A
amostragem é simulada sobre um \textit{corpus} já anotado, sem custo
diferencial entre estratos --- caso o custo de rotulagem variasse, a
alocação ótima incluiria um termo $\sqrt{c_h}$ no denominador
(Cochran §5.5).
