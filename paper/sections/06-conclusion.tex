...
dependente do estimador adotado. \textbf{(Q3)} Sob replicação Monte Carlo ($B=500$), a média estratificada
simples e a regressão combinada empatam pelo menor erro quadrático médio
($0{,}00115$). O estimador tipo razão combinado, embora exiba o menor erro
absoluto contra a verdade populacional em uma realização única ($0{,}69\%$
de $\bar{Y}$), não preserva essa vantagem após replicação. A baixíssima
correlação populacional ($\rho_{xy} \approx 0{,}011$) entre o auxiliar
escolhido e a variável-alvo limita drasticamente o ganho da regressão sobre
a média estratificada --- conforme previsto por Cochran~\cite{cochran1977}~§7.4.
Esta é uma limitação reconhecida da escolha experimental, e o uso de
auxiliares mais informativos é apontado como trabalho futuro.
...
\paragraph{Contribuição à universidade.} O trabalho integra a formação
oferecida pelo Programa de Pós-Graduação em Informática da Universidade de
Brasília, articulando duas tradições do currículo --- estatística inferencial
e processamento de língua natural --- em um \textit{pipeline} aplicado e
reproduzível. O material (código, \textit{notebooks}, suíte de testes) é
disponibilizado como recurso didático para disciplinas de Amostragem,
Aprendizado de Máquina e Ética em IA do programa.

\paragraph{Contribuição ao grupo de pesquisa.} O \textit{pipeline} estabelece
um instrumento metodológico reutilizável para a linha de pesquisa em
\textit{Auditoria de Sistemas de IA} do grupo, abrindo extensões diretas a
trabalhos em curso sobre detecção de viés em modelos de linguagem treinados
em português brasileiro e sobre avaliação inferencial de saídas de LLMs.
