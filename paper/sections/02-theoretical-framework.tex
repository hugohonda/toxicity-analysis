...
alocação \textit{proporcional} $n_h = n \cdot W_h$. Quando os $\sigma_h$
diferem materialmente entre estratos, Neyman pode reduzir substancialmente o
erro-padrão para o mesmo $n$ --- esta é a hipótese empiricamente avaliada na
...
O ToLD-Br~\cite{leite2020} consiste em $21{,}000$ \textit{tweets} em
português brasileiro, anotados por três avaliadores em seis categorias de
dano: \textit{homophobia}, \textit{obscene}, \textit{insult}, \textit{racism},
\textit{misogyny}, \textit{xenophobia}. Cada anotação é um inteiro
$v_{ij} \in \{0,1,2,3\}$ representando o número de avaliadores que sinalizou a
categoria $j$ para o \textit{tweet} $i$. A binarização operacional consagrada
na literatura é $\mathbb{1}[v_{ij} \geq 2]$ (regra de maioria), mas o esquema
ordinal preserva informação sobre o grau de concordância, útil tanto para
estratificação quanto para diagnóstico de ruído de
rotulagem~\cite{leite2020}.
